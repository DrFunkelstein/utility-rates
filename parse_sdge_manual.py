...
def parse_sdge_pdf(pdf_path):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        # 1. Identify Plan ID (Exclusive matching)
        plan_match = re.search(r"Schedule\s+([A-Z0-9-]+)", text)
        if plan_match:
            results["plan_id"] = plan_match.group(1)
            # 'DR' is the specific code for the non-TOU tiered plan
        ...
        if table:
            ...
            for row in table:
                ...
                if current_season:
                    # We only care about the 'Total Electric Rate' column
                    total_rate = extract_decimal(row[-1])
                    
                    # Safety: If the last column isn't a valid decimal rate, skip the row
                    if total_rate <= 0.01: continue 

                    if results["is_tiered"]:
                        # TIERED (Schedule DR) EXCLUSIVE LOGIC
                        # Look for 'Up to' vs 'Above' to distinguish Tier 1 and Tier 2
                        if "Up to" in row_str:
                            print(f"    Found Tier 1 ({current_season}): {total_rate}")
                            results[current_season]["on"] = total_rate
                        elif "Above" in row_str:
...
def main():
    ...
    for filename in os.listdir(UPLOAD_DIR):
        ...
        def update_val(category, bin_name, current_val, new_val):
            ...
            # Ensure new_val is valid and different from current
        ...
        # 1. Update Fixed Charges
        p["dailyServiceCharge"] = update_val("Fixed", "Service Charge", p.get("dailyServiceCharge", 0), pdf_data["service_charge"])

        # 2. Update Seasonal Tiers
        for season in ["summer", "winter"]:
            p[season]["onPeak"] = update_val(season.capitalize(), "T1/On", p[season].get("onPeak"), pdf_data[season]["on"])
            p[season]["offPeak"] = update_val(season.capitalize(), "T2/Off", p[season].get("offPeak"), pdf_data[season]["mid"])
            p[season]["superOffPeak"] = update_val(season.capitalize(), "T2/SuperOff", p[season].get("superOffPeak"), pdf_data[season]["off"])

        # 3. Update Global Baseline Credit
